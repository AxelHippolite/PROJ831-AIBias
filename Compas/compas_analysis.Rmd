...
RK_violence_age <-  subset(RK_violence, select = c(Age, ScoreText, AgeGroup)) 
...
We will now focus our interest on the recidivism risk dataset. We will try to determine which variables are most relevant for predicting the probability of recidivism. First, we perform a logistic regression on the dataset against the recidivism score. We decide to separate the individuals with a low chance of recidivism on one side and those with a medium and high on the other.

```{r}

RK_recidivism<- subset(RK_recidivism, select = -c(DisplayText,ScoreText))
...
# Appliquer la fonction step backward
#step_model <- step(model, direction = "backward")#legal status and age removed
...
formattable(confusion_df, list(
  `1` = color_tile("#ffffff", "#2a9fd6"),
  `0` = color_tile("#ffffff", "#ff5722")
))

confusion_df
```

-   2234 observations ont été correctement classées comme négatives par le modèle (valeur prédite = 0, valeur réelle = 0).

-   67 observations ont été classées comme positives par le modèle alors qu'elles étaient en réalité négatives (valeur prédite = 1, valeur réelle = 0), ce qui correspond à des faux positifs.

-   296 observations ont été classées comme négatives par le modèle alors qu'elles étaient en réalité positives (valeur prédite = 0, valeur réelle = 1), ce qui correspond à des faux négatifs.

-   1459 observations ont été correctement classées comme positives par le modèle (valeur prédite = 1, valeur réelle = 1).
...
# Convertir la variable DecileScore en facteur avec les niveaux spécifiés
...
# Exécuter la régression logistique multinomiale
...
confusion_table <- formattable(confusion_df, align = "c", list(
  `low` = color_tile("#FFFFFF", "#2A9FD6"),
  `medium` = color_tile("#FFFFFF", "#2A9FD6"),
  `high` = color_tile("#FFFFFF", "#2A9FD6")
))
confusion_matrix
confusion_table
```
