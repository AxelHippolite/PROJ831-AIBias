...
```{r}
install.packages(c("CGPfunctions", "treemap", "ggplot2", "formattable", "nnet", "caret", "MASS", "broom.helpers", "GGally", "RColorBrewer"))
```
...
```{r}
people_data <- read.csv("people-compas.csv.csv")

recidiv_individuals <- subset(people_data, is_recid == "1")
recidiv_individuals$age_grp <- cut(recidiv_individuals$age, age_groups, labels = c("<25", "25-30", "30-40", "40-50", "50-60", ">60"))
...
ggplot(recidiv_individuals,aes(x=factor(age_grp)))+
  ...
  labs(title="Age group of recidivist individuals", x="Age groupe", y = "Number of individuals") + geom_text(aes(label=..count..),stat='count',position=position_dodge(0.9),vjust=-0.2) + theme_minimal()
```
